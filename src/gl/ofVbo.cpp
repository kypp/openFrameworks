...
#else
	if(!vaoChecked){
		supportVAOs = ofGetGLProgrammableRenderer().get();// || glewIsSupported("GL_ARB_vertex_array_object"); <- this should work but has false positives on some cards like emulation in vm's
		vaoChecked = true;
	}
#endif
